...
public class RoleController {
    ...
    @PostMapping
    @ApiOperation("修改角色信息")
    public ResponseResult update(@RequestBody Role role,@PathVariable @ApiParam(value="角色id") Long id){
        role.setId(id);
    ...
    @RequestParam @ApiParam(value="角色状态L1-启用 0-禁用",allowableValues="0,1") Integer status){
    ...
    public ResponseResult list(@RequestParam(required = false) @ApiParam(value = "角色的名称") String keyword,
            ...
            if (!StringUtils.isEmpty(keyword)){
                queryWrapper.eq("name",keyword);//紧缺查找  如果需要模糊查询 eq换成like就行
            }
    ...
    public ResponseResult delete(@RequestBody @ApiParam(value = "角色id列表",example="1,2,3") String ids){
        QueryWrapper<Role> queryWrapper = new QueryWrapper<>();
    ...
    public ResponseResult allocMenu(@RequestParam @ApiParam(value="角色id",required = true) Long roleId,
                                    @RequestParam @ApiParam(value="菜单id",required = true) String menuIds){
