...
public class RoleController {
    ...
    public ResponseResult updateStatus(@PathVariable @ApiParam(value = "角色id",required = true) Long id,
                                       @PathVariable @ApiParam(value = "角色状态:1-启用，0-禁用",
                                               allowableValues = "0,1") Integer status){
    ...
    public ResponseResult listMenu(@PathVariable @ApiParam(value = "角色id列表",required = true)Long roleId){


        return new ResponseResult(200,"查询菜单成功",menuService.listByRoleId(roleId));
    }
    ...
    public ResponseResult list(@RequestParam(required = false) @ApiParam(value = "角色名称") String keyword,
        ...
        if (!StringUtils.isEmpty(keyword))
            queryWrapper.like("name",keyword);//精确查找  如果需要模糊查询  eq换成like即可
